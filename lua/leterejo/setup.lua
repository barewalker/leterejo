...
local function wrapper_script()
  return {
    ...
    'cd "$path"',
    "",
    "# Say when another gmi already has the repository, rather than let this one",
    "# raise. lieer takes its lock without waiting and then exits with a Python",
    "# traceback, which on some desktops becomes a crash report dialog — for a",
    "# collision that is ordinary: the editor fetches on a timer, so a gmi run by",
    "# hand meets it sooner or later. Nothing is wrong when this happens; the",
    "# other one is simply still going.",
    "case $1 in",
    "    gmi)",
    '        if [ -e "$path/.lock" ] && command -v flock >/dev/null 2>&1; then',
    '            if ! flock -n "$path/.lock" true 2>/dev/null; then',
    '                echo "leterejo-env: $account: another gmi is using $path" >&2',
    '                echo "  it is probably the editor\'s scheduled fetch; try again in a moment" >&2',
    "                exit 1",
    "            fi",
    "        fi",
    "        ;;",
